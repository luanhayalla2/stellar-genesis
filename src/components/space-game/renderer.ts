...
export function drawExplosions(ctx: CanvasRenderingContext2D, explosions: Explosion[], camX: number, camY: number) {
  for (const e of explosions) {
    for (const p of e.particles) {
      const ex = p.x - camX, ey = p.y - camY;
      ctx.beginPath();
      ctx.arc(ex, ey, p.size * p.life, 0, Math.PI * 2);
...
export function drawHUD(ctx: CanvasRenderingContext2D, ship: Ship, score: number, w: number, h: number, planets: Planet[], gameState: GameState, waveState: WaveState, onlinePlayers: number) {
  ...
  if (ship.doubleShot) {
    ...
    ctx.font = "12px monospace";
    ctx.fillText(`🔫 TIRO DUPLO ${Math.ceil(ship.doubleShotTimer / 60)}s`, 20, puY);
    puY += 18;
  }
  if (ship.speedBoost) {
  ...
  const mmY = 15;
  const mmScale = 6000;
  ctx.fillStyle = "hsla(240, 20%, 5%, 0.7)";
  ctx.strokeStyle = "hsla(200, 50%, 40%, 0.4)";
  ctx.lineWidth = 1;
  ctx.fillRect(mmX, mmY, mmSize, mmSize);
  ctx.strokeRect(mmX, mmY, mmSize, mmSize);
  for (const p of planets) {
    const mx = mmX + mmSize / 2 + ((p.x - ship.x) / mmScale) * mmSize;
    const my = mmY + mmSize / 2 + ((p.y - ship.y) / mmScale) * mmSize;
    if (mx > mmX && mx < mmX + mmSize && my > mmY && my < mmY + mmSize) {
      ctx.beginPath();
      ctx.arc(mx, my, Math.max(2, p.radius / 30), 0, Math.PI * 2);
      ctx.fillStyle = `hsla(${p.hue}, ${p.sat}%, ${p.light + 20}%, 0.8)`;
      ctx.fill();
    }
  }
  ctx.beginPath();
  ctx.arc(mmX + mmSize / 2, mmY + mmSize / 2, 2, 0, Math.PI * 2);
  ctx.fillStyle = "hsla(120, 90%, 60%, 1)";
  ctx.fill();
